...
public class HmsDA {
	 ...
	 * Statement stmt2 = con.createStatement(); ResultSet set2 =
	 * stmt2.executeQuery("SELECT * FROM employee WHERE eid="+eid);
	 ...
	 * emp.setDob(set2.getString("dob")); emp.setPhone(set2.getString("phone"));
	 * emp.setSalary(set2.getInt("salary")); emp.setUid(set2.getInt("uid"));
	 * emp.setEid(set2.getInt("eid")); emp.setUser(getUser(emp.getUid())); }
	 * catch(SQLException e){ e.printStackTrace(); } return emp; }
	 * 
	 * 
	 * 
	 * public Category getCategory(String name){ Category cat = new Category();
	 * try{ Statement stmt4 = con.createStatement(); ResultSet set4 =
	 * stmt4.executeQuery("SELECT * FROM category WHERE name='"+name+"'");
	 * if(!set4.next()) return null;
	 * 
	 * cat.setCatid(set4.getInt("catid")); cat.setName(set4.getString("name"));
	 * } catch(SQLException e){ e.printStackTrace(); } return cat; }
	 * 
	 * 
	 * 
	 * public Patient getPatient(int pid) throws SQLException{ Patient patient =
	 * null; ResultSet set =
	 * con.createStatement().executeQuery("SELECT * FROM patient WHERE pid="+pid
	 * ); while(set.next()){ patient = new Patient();
	 * patient.setPid(set.getInt("pid"));
	 * patient.setName(set.getString("name"));
	 * patient.setGender(set.getString("gender"));
	 * patient.setDob(set.getString("dob"));
	 * patient.setType(set.getString("type"));
	 * patient.setCatid(set.getInt("catid")); patient.setDid(set.getInt("did"));
	 * 
	 * patient.setCategory(getCategory(patient.getCatid()));
	 * 
	 * if(patient.getDid()==0){ patient.setDoctor(null); } else {
	 * patient.setDoctor(getDoctor(patient.getDid())); } } return patient; }
	 * 
	 * 
	 * public ArrayList<Patient> getPatientList(){
	 * 
	 * ArrayList<Patient> patientList = new ArrayList<Patient>();
	 * 
	 * try{ Statement stmt = con.createStatement(); ResultSet set =
	 * stmt.executeQuery("SELECT * FROM patient");
	 * 
	 * patientList = getPatientListFromSet(set); } catch(SQLException ex){
	 * ex.printStackTrace(); } return patientList; }
	 * 
	 * public ArrayList<Patient> getPatientListFromSet(ResultSet set) throws
	 * SQLException{
	 * 
	 * ArrayList<Patient> patientList = new ArrayList<Patient>();
	 * 
	 * while(set.next()){ Patient patient = new Patient();
	 * patient.setPid(set.getInt("pid"));
	 * patient.setName(set.getString("name"));
	 * patient.setDob(set.getString("dob"));
	 * patient.setGender(set.getString("gender"));
	 * patient.setType(set.getString("type"));
	 * 
	 * int catid = set.getInt("catid"); int did = set.getInt("did");
	 * 
	 * patient.setCatid(catid); patient.setDid(did);
	 * 
	 * patient.setCategory(getCategory(catid));
	 * 
	 * if(did==0) patient.setDoctor(null); else {
	 ...
	 * 
	 * 
	 * public ArrayList<Indoor> getIndoorList(){ ArrayList<Indoor> indoorList =
	 * new ArrayList<Indoor>();
	 * 
	 * try{ Statement stmt = con.createStatement(); ResultSet set =
	 * stmt.executeQuery("SELECT * FROM indoor"); while(set.next()){ Indoor
	 * indoor = new Indoor();
	 * 
	 * indoor.setIpid(set.getInt("ipid"));
	 * indoor.setDisease(set.getString("disease"));
	 * indoor.setPid(set.getInt("pid")); indoor.setRid(set.getInt("rid"));
	 * indoor.setStatus(set.getString("status"));
	 * indoor.setPatient(getPatient(indoor.getPid()));
	 * 
	 * if(indoor.getRid()==0){ indoor.setRoom(null); } else {
	 * indoor.setRoom(getRoom(indoor.getRid())); }
	 * 
	 * indoorList.add(indoor);
	 * 
	 * } } catch(SQLException e){ e.printStackTrace(); } return indoorList; }
	 * 
	 * 
	 * 
	 ...
	 * con.prepareStatement("SELECT * FROM prescription"); ResultSet set =
	 * stmt.executeQuery();
	 * 
	 * 
	 * return getPresListFromSet(set);
	 ...
	 * PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM prescription WHERE prid=?");
	 * stmt.setInt(1, prid); ResultSet set = stmt.executeQuery();
	 * 
	 * if(!set.next()) return null;
	 * 
	 * Prescription pres = new Prescription(); pres.setPrid(prid);
	 * pres.setDid(set.getInt("did")); pres.setMid(set.getInt("mid"));
	 * pres.setPid(set.getInt("pid")); pres.setDosage(set.getString("dosage"));
	 * 
	 ...
	 * }
	 * 
	 * public ArrayList<Prescription> getPresListFromSet(ResultSet set) throws
	 * SQLException{
	 * 
	 * ArrayList<Prescription> presList = new ArrayList<Prescription>();
	 * 
	 * while(set.next()){ Prescription p = new Prescription();
	 * p.setPrid(set.getInt("prid")); p.setDid(set.getInt("did"));
	 * p.setDoctor(getDoctor(p.getDid())); p.setDosage(set.getString("dosage"));
	 * p.setMid(set.getInt("mid")); p.setMedicine(getMedicine(p.getMid()));
	 * p.setPid(set.getInt("pid")); p.setPatient(getPatient(p.getPid()));
	 * 
	 * presList.add(p); }
	 * 
	 * return presList;
	 ...
	 * }
	 * 
	 * public ArrayList<Patient> getDocPatientsList(int uid) throws
	 * SQLException{
	 * 
	 * int eid = getEidByUid(uid);
	 * 
	 * PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM doctor WHERE eid=?"); stmt.setInt(1,
	 * eid); ResultSet set = stmt.executeQuery(); if(!set.next()) return null;
	 * 
	 * int catid = set.getInt("catid");
	 * 
	 * stmt = con.prepareStatement("SELECT * FROM patient WHERE catid=?");
	 * stmt.setInt(1, catid); set = stmt.executeQuery();
	 * 
	 * return getPatientByCatid(catid);
	 * 
	 * }
	 * 
	 * public ArrayList<Prescription> getDocPresList(int uid) throws
	 * SQLException{
	 * 
	 * int eid = getEidByUid(uid);
	 * 
	 ...
	 * 
	 * stmt = con.prepareStatement("SELECT * FROM prescription WHERE did=?");
	 * stmt.setInt(1, did); set = stmt.executeQuery();
	 * 
	 * return getPresListFromSet(set);
	 * 
	 * }
	 * 
	 * public ArrayList<Prescription> getPatientPres(int pid)throws
	 * SQLException{ PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM prescription WHERE pid=?");
	 * stmt.setInt(1, pid); ResultSet set = stmt.executeQuery();
	 * 
	 * return getPresListFromSet(set); }
	 * 
	 * public ArrayList<Patient> getPatientByCatid(int catid) throws
	 * SQLException{ PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM patient WHERE catid=?");
	 * stmt.setInt(1, catid); ResultSet set = stmt.executeQuery();
	 * ArrayList<Patient> patList= getPatientListFromSet(set);
	 * 
	 * return patList; }
	 * 
	 * public Medicine getMedicine(int mid) throws SQLException{
	 * PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM medicine WHERE mid=?");
	 * stmt.setInt(1, mid); ResultSet set = stmt.executeQuery();
	 * 
	 * Medicine med = new Medicine(); if(!set.next()) return null;
	 * 
	 * med.setMid(mid); med.setName(set.getString("name"));
	 * med.setPrice(set.getInt("price"));
	 * 
	 * return med;
	 * 
	 ...
	 * 
	 * PreparedStatement stmt = con.
	 * prepareStatement("INSERT INTO employee(firstname,lastname,gender,dob,salary,phone,uid) VALUES(?,?,?,?,?,?,?) "
	 * , Statement.RETURN_GENERATED_KEYS);
	 * stmt.setString(1,employee.getFirstname());
	 * stmt.setString(2,employee.getLastname());
	 * stmt.setString(3,employee.getGender()); stmt.setString(4,
	 * employee.getDob()); stmt.setInt(5,employee.getSalary());
	 * stmt.setString(6,employee.getPhone()); stmt.setInt(7,employee.getUid());
	 * 
	 * stmt.executeUpdate(); ResultSet keys = stmt.getGeneratedKeys();
	 * if(keys!=null && keys.next()) eid=keys.getInt(1); employee.setEid(eid);
	 * return eid; }
	 * 
	 * public int addUser(User user) throws SQLException{ int uid=0;
	 * PreparedStatement stmt = con.
	 * prepareStatement("INSERT INTO users(username,password,type) VALUES(?,?,?) "
	 * , Statement.RETURN_GENERATED_KEYS); stmt.setString(1,
	 * user.getUsername()); stmt.setString(2, user.getPassword());
	 * stmt.setString(3, user.getType());
	 * 
	 * stmt.executeUpdate();
	 * 
	 * ResultSet keys = stmt.getGeneratedKeys(); if(keys!=null && keys.next())
	 * uid = keys.getInt(1); user.setUid(uid); return uid; }
	 * 
	 * 
	 * 
	 ...
	 * 
	 * PreparedStatement stmt =
	 * con.prepareStatement("DELETE FROM patient WHERE pid=?"); stmt.setInt(1,
	 * pid); stmt.execute();
	 * 
	 * }
	 * 
	 ...
	 * 
	 * PreparedStatement stmt = con.
	 * prepareStatement("INSERT INTO patient(name,gender,dob,catid) VALUES(?,?,?,?)"
	 * ,Statement.RETURN_GENERATED_KEYS); stmt.setString(1,p.getName());
	 * stmt.setString(2,p.getGender()); stmt.setString(3,p.getDob());
	 * stmt.setInt(4, p.getCatid()); stmt.executeUpdate();
	 * 
	 * ResultSet keys = stmt.getGeneratedKeys(); int pid = 0; if(keys!=null &&
	 * keys.next()) pid = keys.getInt(1);
	 * 
	 * p.setPid(pid); return p; }
	 * 
	 * public void updateUser(User user) throws SQLException{ PreparedStatement
	 * stmt = con.
	 * prepareStatement("UPDATE users SET username=?, password=?, type=? WHERE uid=?"
	 * ); stmt.setString(1, user.getUsername()); stmt.setString(2,
	 * user.getPassword()); stmt.setString(3, user.getType()); stmt.setInt(4,
	 * user.getUid()); stmt.execute(); }
	 * 
	 * 
	 * 
	 * 
	 ...
	 * PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM room WHERE nid=?"); stmt.setInt(1,
	 * nid); ResultSet set = stmt.executeQuery();
	 * 
	 * ArrayList<Room> roomList = new ArrayList<Room>();
	 * 
	 * while(set.next()){ Room room = new Room();
	 * room.setRid(set.getInt("rid"));
	 * room.setAvailableBeds(set.getInt("availablebeds"));
	 * room.setNid(set.getInt("nid")); room.setNurse(getNurse(room.getNid()));
	 * room.setTotalBeds(set.getInt("totalbeds"));
	 * 
	 * roomList.add(room); }
	 * 
	 * return roomList; }
	 * 
	 * public int getEidByUid(int uid) throws SQLException{ PreparedStatement
	 * stmt = con.prepareStatement("SELECT eid FROM employee WHERE uid=?");
	 * stmt.setInt(1, uid); ResultSet set = stmt.executeQuery(); if(!set.next())
	 * return 0;
	 * 
	 ...
	 * con.prepareStatement("SELECT * FROM nurse WHERE eid=?"); stmt.setInt(1,
	 * eid); ResultSet set = stmt.executeQuery(); set.next(); int nid =
	 * set.getInt("nid"); return getNurseRooms(nid); }
	 * 
	 * public int getDidByUid(int uid) throws SQLException{ int eid =
	 * getEidByUid(uid);
	 * 
	 * PreparedStatement stmt =
	 * con.prepareStatement("SELECT * FROM doctor WHERE eid=?"); stmt.setInt(1,
	 * eid); ResultSet set = stmt.executeQuery();
	 * 
	 * set.next();
	 * 
	 * return set.getInt("did"); }
	 * 
	 * public void addIndoor(Indoor indo) throws SQLException{ PreparedStatement
	 * stmt =
	 * con.prepareStatement("INSERT INTO indoor SET disease=?,pid=?, status=?");
	 * stmt.setString(1, indo.getDisease()); stmt.setInt(2, indo.getPid());
	 * stmt.setString(3,indo.getStatus());
	 * 
	 * stmt.execute();
	 * 
	 * stmt = con.prepareStatement("UPDATE patient SET type=? WHERE pid=?");
	 * stmt.setString(1, "indoor"); stmt.setInt(2, indo.getPid());
	 * stmt.execute();
	 * 
	 * 
	 * }
	 ...
	 * 
	 * PreparedStatement stmt = con.
	 * prepareStatement("INSERT INTO prescription SET dosage=?, pid=?, mid=?, did=?"
	 * , Statement.RETURN_GENERATED_KEYS); stmt.setString(1, p.getDosage());
	 * stmt.setInt(2, p.getPid()); stmt.setInt(3, p.getMid()); stmt.setInt(4,
	 * p.getDid());
	 * 
	 * stmt.executeUpdate(); int prid=0; ResultSet keys =
	 * stmt.getGeneratedKeys(); if(keys!=null && keys.next()) prid =
	 * keys.getInt(1);
	 * 
	 * p.setDoctor(getDoctor(p.getDid()));
	 * p.setMedicine(getMedicine(p.getMid()));
	 * p.setPatient(getPatient(p.getPid())); p.setPrid(prid);
	 * 
	 * } public Prescription updatePrescription(Prescription p)throws
	 * SQLException{ PreparedStatement stmt = con.
	 * prepareStatement("UPDATE prescription SET dosage=?, mid=? WHERE prid=?");
	 * stmt.setString(1, p.getDosage()); stmt.setInt(2, p.getMid());
	 ...
	 * 
	 * public void removePres(int prid) throws SQLException{ PreparedStatement
	 * stmt = con.prepareStatement("DELETE FROM prescription WHERE prid=?");
	 * stmt.setInt(1, prid); stmt.execute(); }
	 * 
	 * public void updateIndoorRid(int ipid, int rid) throws SQLException {
	 * 
	 * PreparedStatement stmt =
	 * con.prepareStatement("UPDATE indoor SET rid=? WHERE ipid=?");
	 * stmt.setInt(1, rid); stmt.setInt(2, ipid); stmt.execute();
	 * 
	 * }
	 * 
	 ...
	 * 
	 * PreparedStatement stmt = con.
	 * prepareStatement("UPDATE patient SET name=?, gender=?, dob=?, type=?, catid=?, did=? WHERE pid=?"
	 * ); stmt.setString(1, p.getName()); stmt.setString(2, p.getGender());
	 * stmt.setString(3, p.getDob()); stmt.setString(4, p.getType());
	 * stmt.setInt(5, p.getCatid()); stmt.setInt(6, p.getDid()); stmt.setInt(7,
	 * p.getPid()); stmt.execute();
	 * 
	 * }
	 * 
	 * public Medicine addMedicine(Medicine med) throws SQLException {
	 * PreparedStatement stmt =
	 * con.prepareStatement("INSERT INTO medicine SET name=?, price=?",
	 * Statement.RETURN_GENERATED_KEYS); stmt.setString(1, med.getName());
	 * stmt.setInt(2, med.getPrice());
	 * 
	 * stmt.executeUpdate();
	 * 
	 * int mid=0; ResultSet keys = stmt.getGeneratedKeys(); if(keys!=null &&
	 * keys.next()) mid = keys.getInt(1);
	 * 
	 * med.setMid(mid);
	 * 
	 * return med;
	 ...
	 * PreparedStatement stmt =
	 * con.prepareStatement("UPDATE medicine SET name=?, price=? WHERE mid=?");
	 * stmt.setString(1, med.getName()); stmt.setInt(2, med.getPrice());
	 * stmt.setInt(3,med.getMid());
	 * 
	 * stmt.execute(); return med; }
	 * 
	 * public void removeMedicine(int mid) throws SQLException {
	 * 
	 * PreparedStatement stmt =
	 * con.prepareStatement("DELETE FROM medicine WHERE mid=?"); stmt.setInt(1,
	 * mid); stmt.execute();
	 * 
	 * }
	 * 
	 * 
	 * <<<<<<< HEAD public Category addCategory(Category cat) throws
	 * SQLException {
	 * 
	 * PreparedStatement stmt =
	 * con.prepareStatement("INSERT INTO category SET name=?",Statement.
	 * RETURN_GENERATED_KEYS); stmt.setString(1, cat.getName());
	 * 
	 * int catid=0; stmt.executeUpdate(); ResultSet keys =
	 * stmt.getGeneratedKeys(); if(keys!=null && keys.next())
	 * catid=keys.getInt(1);
	 * 
	 * cat.setCatid(catid); return cat; } =======
	 * 
	 * 
	 * 
	 ...
	 * 
	 * 
	 * public void decreaseBed(int rid) throws SQLException { PreparedStatement
	 * stmt = con.prepareStatement("SELECT * FROM room WHERE rid=?");
	 * stmt.setInt(1, rid); ResultSet set = stmt.executeQuery();
	 * 
	 * int availablebeds=0;
	 * 
	 ...
	 * 
	 * stmt =
	 * con.prepareStatement("UPDATE room SET availablebeds=? WHERE rid=?");
	 * stmt.setInt(1, availablebeds); stmt.setInt(2, rid); stmt.execute();
	 * 
	 * }
	 */
 ...
 * public Employee getEmployee(int eid){ Employee emp = new Employee(); try{
 * Statement stmt2 = con.createStatement(); ResultSet set2 =
 * stmt2.executeQuery("SELECT * FROM employee WHERE eid="+eid); if(!set2.next())
 * return null;
 * 
 * emp.setFirstname(set2.getString("firstname"));
 * emp.setLastname(set2.getString("lastname"));
 * emp.setGender(set2.getString("gender")); emp.setDob(set2.getString("dob"));
 * emp.setPhone(set2.getString("phone")); emp.setSalary(set2.getInt("salary"));
 * emp.setUid(set2.getInt("uid")); emp.setEid(set2.getInt("eid"));
 * emp.setUser(getUser(emp.getUid())); } catch(SQLException e){
 ...
 * try {
 * 
 * stmt = con.createStatement(); ResultSet set =
 * stmt.executeQuery("SELECT * FROM employee WHERE category_catid =" + did);
 * 
 * 
 * while (set.next()) { Employee doc = new Employee();
 * doc.setEid(set.getInt("eid")); doc.setName(set.getString("name"));
 * doc.setUsername(set.getString("username"));
 * doc.setPasswd(set.getString("passwd"));
 * doc.setGender(set.getString("gender")); if (set.getString("birth") == null) {
 * doc.setBirth(""); } else { doc.setBirth(set.getString("birth")); }
 * doc.setPhone(set.getString("phone")); doc.setSalary(set.getInt("salary"));
 * doc.setCatid(set.getInt("category_catid")); docList.add(doc); } } catch
 ...
 * prepareStatement("INSERT INTO patient(name,gender,dob,catid) VALUES(?,?,?,?)"
 * ,Statement.RETURN_GENERATED_KEYS); stmt.setString(1,p.getName());
 * stmt.setString(2,p.getGender()); stmt.setString(3,p.getDob()); stmt.setInt(4,
 * p.getCatid()); stmt.executeUpdate();
 * 
 * ResultSet keys = stmt.getGeneratedKeys(); int pid = 0; if(keys!=null &&
 * keys.next()) pid = keys.getInt(1);
 * 
 ...
 * 
 * 
 * public int getEidByUid(int uid) throws SQLException{ PreparedStatement stmt =
 * con.prepareStatement("SELECT eid FROM employee WHERE uid=?"); stmt.setInt(1,
 * uid); ResultSet set = stmt.executeQuery(); if(!set.next()) return 0;
 * 
 * int eid = set.getInt("eid");
 * 
 * return eid; }
 ...
 * 
 * PreparedStatement stmt; stmt =
 * con.prepareStatement("SELECT * FROM nurse WHERE eid=?"); stmt.setInt(1, eid);
 * ResultSet set = stmt.executeQuery(); set.next(); int nid = set.getInt("nid");
 ...
 * 
 * stmt = con.prepareStatement("UPDATE patient SET type=? WHERE pid=?");
 * stmt.setString(1, "indoor"); stmt.setInt(2, indo.getPid()); stmt.execute();
 * 
 * 
 * }
 * 
 ...
 * p.getDid());
 * 
 * stmt.executeUpdate(); int prid=0; ResultSet keys = stmt.getGeneratedKeys();
 * if(keys!=null && keys.next()) prid = keys.getInt(1);
 * 
 * p.setDoctor(getDoctor(p.getDid())); p.setMedicine(getMedicine(p.getMid()));
 * p.setPatient(getPatient(p.getPid())); p.setPrid(prid);
 * 
 * } public Prescription updatePrescription(Prescription p)throws SQLException{
 * PreparedStatement stmt =
 * con.prepareStatement("UPDATE prescription SET dosage=?, mid=? WHERE prid=?");
 * stmt.setString(1, p.getDosage()); stmt.setInt(2, p.getMid()); stmt.setInt(3,
 * p.getPrid());
 * 
 * stmt.execute();
 * 
 * return getPrescription(p.getPrid()); }
 * 
 * public void removePres(int prid) throws SQLException{ PreparedStatement stmt
 * = con.prepareStatement("DELETE FROM prescription WHERE prid=?");
 * stmt.setInt(1, prid); stmt.execute(); }
 * 
 * 
 * 
 ...
 * prepareStatement("UPDATE patient SET name=?, gender=?, dob=?, type=?, catid=?, did=? WHERE pid=?"
 * ); stmt.setString(1, p.getName()); stmt.setString(2, p.getGender());
 * stmt.setString(3, p.getDob()); stmt.setString(4, p.getType()); stmt.setInt(5,
 * p.getCatid()); stmt.setInt(6, p.getDid()); stmt.setInt(7, p.getPid());
 * stmt.execute();
 * 
 * }
 * 
 ...
 * 
 * 
 * public Category getCategory(String name){ Category cat = new Category(); try{
 * Statement stmt4 = con.createStatement(); ResultSet set4 =
 * stmt4.executeQuery("SELECT * FROM category WHERE name='"+name+"'");
 * if(!set4.next()) return null;
 * 
 * cat.setCatid(set4.getInt("catid")); cat.setName(set4.getString("name")); }
 ...
 * con.createStatement().executeQuery("SELECT * FROM patient WHERE pid="+pid );
 * while(set.next()){ patient = new Patient();
 * patient.setPid(set.getInt("pid")); patient.setName(set.getString("name"));
 * patient.setGender(set.getString("gender"));
 * patient.setDob(set.getString("dob")); patient.setType(set.getString("type"));
 * patient.setCatid(set.getInt("catid")); patient.setDid(set.getInt("did"));
 * 
 * patient.setCategory(getCategory(patient.getCatid()));
 * 
 ...
 * patient.setCategory(getCategory(catid));
 * 
 * if(did==0) patient.setDoctor(null); else { patient.setDoctor(getDoctor(did));
 * }
 * 
 * patientList.add(patient); }
 * 
 ...
 * 
 * 
 * public ArrayList<Indoor> getIndoorList(){ ArrayList<Indoor> indoorList = new
 * ArrayList<Indoor>();
 * 
 * try{ Statement stmt = con.createStatement(); ResultSet set =
 * stmt.executeQuery("SELECT * FROM indoor"); while(set.next()){ Indoor indoor =
 * new Indoor();
 * 
 * indoor.setIpid(set.getInt("ipid"));
 * indoor.setDisease(set.getString("disease"));
 ...
 * PreparedStatement stmt =
 * con.prepareStatement("SELECT * FROM doctor WHERE eid=?"); stmt.setInt(1,
 * eid); ResultSet set = stmt.executeQuery(); if(!set.next()) return null; int
 * did = set.getInt("did");
 * 
 * stmt = con.prepareStatement("SELECT * FROM prescription WHERE did=?");
 * stmt.setInt(1, did); set = stmt.executeQuery();
 ...
 * public ArrayList<Patient> getPatientByCatid(int catid) throws SQLException{
 * PreparedStatement stmt =
 * con.prepareStatement("SELECT * FROM patient WHERE catid=?"); stmt.setInt(1,
 * catid); ResultSet set = stmt.executeQuery(); ArrayList<Patient> patList=
 * getPatientListFromSet(set);
 * 
 * return patList; }
 * 
 * public Medicine getMedicine(int mid) throws SQLException{ PreparedStatement
 * stmt = con.prepareStatement("SELECT * FROM medicine WHERE mid=?");
 * stmt.setInt(1, mid); ResultSet set = stmt.executeQuery();
 * 
 * Medicine med = new Medicine(); if(!set.next()) return null;
 * 
 ...
 * stmt.setString(1,employee.getFirstname());
 * stmt.setString(2,employee.getLastname());
 * stmt.setString(3,employee.getGender()); stmt.setString(4, employee.getDob());
 * stmt.setInt(5,employee.getSalary()); stmt.setString(6,employee.getPhone());
 * stmt.setInt(7,employee.getUid());
 * 
 * stmt.executeUpdate(); ResultSet keys = stmt.getGeneratedKeys(); if(keys!=null
 * && keys.next()) eid=keys.getInt(1); employee.setEid(eid); return eid; }
 * 
 * public int addUser(User user) throws SQLException{ int uid=0;
 * PreparedStatement stmt = con.
 * prepareStatement("INSERT INTO users(username,password,type) VALUES(?,?,?) " ,
 * Statement.RETURN_GENERATED_KEYS); stmt.setString(1, user.getUsername());
 * stmt.setString(2, user.getPassword()); stmt.setString(3, user.getType());
 * 
 * stmt.executeUpdate();
 * 
 * ResultSet keys = stmt.getGeneratedKeys(); if(keys!=null && keys.next()) uid =
 * keys.getInt(1); user.setUid(uid); return uid; }
 * 
 * 
 * 
 * public void deletePatient(int pid) throws SQLException{
 * 
 * PreparedStatement stmt =
 * con.prepareStatement("DELETE FROM patient WHERE pid=?"); stmt.setInt(1, pid);
 * stmt.execute();
 * 
 * }
 * 
 ...
 * p.setPid(pid); return p; }
 * 
 * public void updateUser(User user) throws SQLException{ PreparedStatement stmt
 * = con.
 * prepareStatement("UPDATE users SET username=?, password=?, type=? WHERE uid=?"
 * ); stmt.setString(1, user.getUsername()); stmt.setString(2,
 * user.getPassword()); stmt.setString(3, user.getType()); stmt.setInt(4,
 ...
 * public ArrayList<Room> getNurseRooms(int nid) throws SQLException{
 * PreparedStatement stmt =
 * con.prepareStatement("SELECT * FROM room WHERE nid=?"); stmt.setInt(1, nid);
 * ResultSet set = stmt.executeQuery();
 * 
 * ArrayList<Room> roomList = new ArrayList<Room>();
 * 
 * while(set.next()){ Room room = new Room(); room.setRid(set.getInt("rid"));
 * room.setAvailableBeds(set.getInt("availablebeds"));
 * room.setNid(set.getInt("nid")); room.setNurse(getNurse(room.getNid()));
 * room.setTotalBeds(set.getInt("totalbeds"));
 * 
 ...
 * 
 * PreparedStatement stmt =
 * con.prepareStatement("UPDATE indoor SET rid=? WHERE ipid=?"); stmt.setInt(1,
 * rid); stmt.setInt(2, ipid); stmt.execute();
 * 
 * }
 * 
 * public void updatePatient(Patient p) throws SQLException {
 ...
 * 
 * 
 * <<<<<<< HEAD public Category addCategory(Category cat) throws SQLException {
 * 
 * PreparedStatement stmt =
 * con.prepareStatement("INSERT INTO category SET name=?",Statement.
 * RETURN_GENERATED_KEYS); stmt.setString(1, cat.getName());
 * 
 * int catid=0; stmt.executeUpdate(); ResultSet keys = stmt.getGeneratedKeys();
 ...
 * 
 * 
 * public void decreaseBed(int rid) throws SQLException { PreparedStatement stmt
 * = con.prepareStatement("SELECT * FROM room WHERE rid=?"); stmt.setInt(1,
 * rid); ResultSet set = stmt.executeQuery();
 * 
 * int availablebeds=0;
 * 
 * if(set.next()) availablebeds = set.getInt("availablebeds");
